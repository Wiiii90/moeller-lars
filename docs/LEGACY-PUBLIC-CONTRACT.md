...
| `site=paintings` | `inc/config.php:57-65` (`paintings` case) | Selects `paintings`, keyword `paintings`, and `inc/entries.php`; metadata is `PAINTINGS`. | Preserve the category meaning as imported data; the new generic `/{category-slug}` route may use its imported slug. | Compare ordered records and fields. |
| `site=drawings` | `inc/config.php:66-74` (`drawings` case) | Selects `drawings` and `entries.php`; metadata is `DRAWINGS`. | Preserve the category meaning as imported data; the new generic route may use its imported slug. | Same fixture comparison as paintings. |
| `site=prints` | `inc/config.php:75-83` (`prints` case) | Selects `prints` and `entries.php`; metadata is `PRINTS`. | Preserve the category meaning as imported data; the new generic route may use its imported slug. | Same fixture comparison as paintings. |
| `site=cyanotype` | `inc/config.php:84-92` (`cyanotype` case) | Dispatcher exposes a category route and `entries.php` query target. | Preserve the observed category meaning only if migration evidence confirms content; use the generic category route. | Verify source query/table availability during migration reconciliation. |
| `site=bichromate` | `inc/config.php:93-101` (`bichromate` case) | Exposes the “salt print & gum bichromate” category label. | Preserve the observed label as migration/editorial data, separately from the factual table list. | Reviewed category mapping. |
| `site=litho` | `inc/config.php:102-110` (`litho` case) | Exposes “etching & lithography”. | Preserve the observed label as migration/editorial data, separately from the factual table list. | Reviewed mapping. |
| `site=photo` | `inc/config.php:111-119` (`photo` case) | Public selector is `photo`, while `$_CATEGORY` is `photos`; query uses the dynamic category path. | Retain the selector/table distinction as migration evidence; do not make either spelling an application constant. | Target-category reconciliation; do not silently rename source evidence. |
| `site=ignis` | `inc/config.php:120-128` (`ignis` case) | Exposes `ignis` and “ignis-serial”. | Preserve the observed meaning as migration/editorial data, separately from the landing tables. | Reviewed mapping. |
| `site=other` | `inc/config.php:129-137` (`other` case) | Exposes “other photography”. | Preserve the observed meaning as migration/editorial data, separately from the landing tables. | Reviewed mapping. |
| `site=vita` | `inc/config.php:138-145` (`vita` case); `inc/vita.php:4-45` | Renders portrait, biography/CV text, exhibition history and disclaimer together. | Preserve the verified biography, portrait, public contact details and disclaimer on `/cv`; migrate the historical exhibition lines into first-class Exhibition records rendered on separate `/exhibitions`. | Compare biography text/portrait and exhibition source rows separately against the reviewed Vita source. |
...
| Category/list ordering | `inc/entries.php:18` (`ORDER BY date DESC`) | Category lists sort by stored date descending. | Preserve the reconciled legacy date-descending display sequence in migrated `position` data; later explicit editorial reordering may intentionally change that sequence. | Compare complete ordered result sets and same-date groups. |
| Home/latest-work query | `inc/landingpage.php:22-34` (`UNION ALL` paintings/drawings/prints, `ORDER BY date DESC LIMIT 1`) | Home combines only the three confirmed tables and selects one newest record. | Preserve the observed winner and category meaning in migration data; the new home uses persisted category presentation data rather than hardcoded slugs. | Compare source query result and target home winner for the approved snapshot. |
...
| Responsive behaviour/breakpoints | `css/style.css:439-458` (550–980), `:460-480` (980–1400), `:482-577` (≤550); `js/resizeimage.js:285-298` (≤500 viewer sizing) | Layout, menu, form, Vita and viewer sizing change at these source breakpoints; small viewer images use window width. | Restore legacy composition as the first visual baseline; accessibility/reliability corrections must not redesign the shell. | Visual/interaction fixtures at representative desktop, tablet and mobile widths. |
| CV/Vita source and portrait | `inc/vita.php:4-37`; `txt/vita.txt`; `inc/config.php:138-145` | Portrait is `res/lars-moeller.jpg`; text is read from `txt/vita.txt` and passed through the legacy BBCode-like formatter. | Migrate biography text/meaning/order losslessly into safe structured CV content; preserve portrait provenance. Historical exhibition lines are migrated separately into Exhibition records. | Text/link/order diff plus portrait checksum/media reconciliation and exhibition reconciliation. |
| Sitemap | `sitemap.xml:9-57` | Static HTTP sitemap lists root, query URLs, Vita and broken `links`; it includes no per-artwork routes. | Regenerate canonical HTTPS sitemap from target routes; include separately enabled `/cv` and `/exhibitions`; omit broken/non-public routes. | Compare classified source inventory to generated target sitemap. |
...
| Canonical/redirect behaviour | `.htaccess:1-4` | Host/index rules redirect to `http://`; direct `index.php` is intended to reach root, but HTTPS is not enforced. | Use one-hop HTTPS/canonical-host behavior for new routes; legacy redirects are optional and evidence-driven, and insecure scheme behavior is not preserved. | HTTP/HTTPS matrix for new routes; assert no loops/fragments/query leakage. |
