                ...
                <div class="journal-workspace__controls is-blog" aria-label="Blog controls">
                    ...
                    <div class="journal-workspace__control-group journal-workspace__selection" x-data="{ open: false }" x-on:keydown.escape.window="open = false">
                        ...
                        <div class="journal-workspace__selection-anchor">
                            ...
                            >
                                Selected posts <span class="journal-workspace__selection-count">{{ count($selectedPostIds) }}</span>
                            </button>
                            <div class="journal-workspace__selection-menu" role="menu" x-show="open" x-cloak x-on:click.outside="open = false">
                                ...
                                <button class="admin-action is-danger" type="button" role="menuitem" wire:click="mountAction('deleteSelectedPosts')" x-on:click="open = false">Delete selected</button>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="journal-workspace__table journal-workspace__table--blog">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" class="journal-workspace__thumb-head">Image</th>
                            ...
                            @foreach ($posts as $post)
                                <tr class="{{ in_array($post['id'], $selectedIds, true) ? 'is-selected' : '' }}" wire:key="blog-post-{{ $post['id'] }}">
                                    <td class="journal-workspace__selection-cell">
                                        <input
                                            type="checkbox"
                                            wire:click="togglePostSelection({{ $post['id'] }})"
                                            @checked(in_array($post['id'], $selectedIds, true))
                                            aria-label="Toggle selection for {{ $post['title'] }}"
                                        >
                                    </td>
                                    <td class="journal-workspace__thumb">
                                    ...
                                    <td class="journal-workspace__identity">
                                        ...
                                        @if ($post['excerpt'])<small>{{ $post['excerpt'] }}</small>@endif
                                    </td>
                                    ...
                                    <td class="journal-workspace__actions">
                                        <div class="admin-toolbar">
                                            ...
                                            @if ($post['public_url'])
                                                <a class="admin-action" href="{{ $post['public_url'] }}" target="_blank" rel="noopener">View</a>
                                            @endif
                                            @switch($post['state'])
                                                @case('published')
                                                    <button class="admin-action" type="button" wire:click="unpublishPost({{ $post['id'] }})">Unpublish</button>
                                                    <button class="admin-action" type="button" wire:click="archivePost({{ $post['id'] }})">Archive</button>
                                                    @break
                                                @case('scheduled')
                                                    <button class="admin-action" type="button" wire:click="restorePostDraft({{ $post['id'] }})">Cancel schedule</button>
                                                    @break
                                                @case('archived')
                                                    <button class="admin-action" type="button" wire:click="restorePostDraft({{ $post['id'] }})">Restore</button>
                                                    @break
                                                @default
                                                    <button class="admin-action" type="button" wire:click="publishPost({{ $post['id'] }})">Publish</button>
                                                    <button class="admin-action" type="button" wire:click="mountAction('schedulePost', { post: {{ $post['id'] }} })">Schedule</button>
                                                    <button class="admin-action" type="button" wire:click="archivePost({{ $post['id'] }})">Archive</button>
                                            @endswitch
                                            ...
                                            <button class="admin-action is-danger" type="button" wire:click="mountAction('deletePost', { post: {{ $post['id'] }} })" @disabled(! $post['can_delete']) title="{{ $post['delete_help'] ?? 'Delete post' }}">Delete</button>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
