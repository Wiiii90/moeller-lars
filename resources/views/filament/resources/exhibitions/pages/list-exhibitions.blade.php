        ...
        <x-admin.section class="journal-workspace__entries" aria-label="Exhibition entries">
            <div class="journal-workspace__surface">
                <div class="journal-workspace__controls is-exhibitions" aria-label="Exhibition controls">
                    ...
                    <label class="journal-workspace__field"><span>Status</span><select wire:model.live="statusFilter"><option value="any">Any</option><option value="draft">Draft</option><option value="published">Published</option><option value="archived">Archived</option></select></label>
                        ...
                        <tbody @if ($dragEnabled) wire:sort="sortExhibition" @endif>
                            @foreach ($exhibitions as $exhibition)
                                <tr class="{{ in_array($exhibition['id'], $selectedIds, true) ? 'is-selected' : '' }}" wire:key="exhibition-{{ $exhibition['id'] }}" @if ($dragEnabled) wire:sort:item="{{ $exhibition['id'] }}" @endif>
                                    ...
                                    <td>{{ str_pad((string) $exhibition['rank'], 2, '0', STR_PAD_LEFT) }}</td>
                                    ...
                                    <td class="journal-workspace__actions"><div class="admin-toolbar">
                                        @if ($exhibition['public_url'])<a class="admin-action" href="{{ $exhibition['public_url'] }}" target="_blank" rel="noopener">View</a>@else<button class="admin-action" type="button" disabled title="No protected/public preview is available for this state">View</button>@endif
                                        <button class="admin-action" type="button" wire:click="mountAction('editExhibition', { exhibition: {{ $exhibition['id'] }} })">Edit</button>
                                        @switch($exhibition['state'])@case('draft')<button class="admin-action" type="button" wire:click="publishExhibition({{ $exhibition['id'] }})">Publish</button>@break @case('published')<button class="admin-action" type="button" wire:click="archiveExhibition({{ $exhibition['id'] }})">Archive</button>@break @case('archived')<button class="admin-action" type="button" wire:click="restoreExhibition({{ $exhibition['id'] }})">Restore</button>@break @endswitch
