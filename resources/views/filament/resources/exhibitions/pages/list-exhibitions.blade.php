            ...
            <div class="journal-workspace__surface">
                <div class="journal-workspace__controls is-exhibitions" aria-label="Exhibition controls">
                    <label class="journal-workspace__field journal-workspace__search">
                        ...
                        <input
                            ...
                            placeholder="Title, venue, place or date"
                            autocomplete="off"
                        >
                    </label>
                    ...
                    <div class="journal-workspace__control-group journal-workspace__selection" x-data="{ open: false }" x-on:keydown.escape.window="open = false">
                        ...
                        <div class="journal-workspace__selection-anchor">
                            ...
                            >
                                Selected exhibitions <span class="journal-workspace__selection-count">{{ count($selectedExhibitionIds) }}</span>
                            </button>
                            <div class="journal-workspace__selection-menu" role="menu" x-show="open" x-cloak x-on:click.outside="open = false">
                                ...
                                <button class="admin-action is-danger" type="button" role="menuitem" wire:click="mountAction('deleteSelectedExhibitions')" x-on:click="open = false">Delete selected</button>
                            </div>
                        </div>
                    </div>
                </div>
                                ...
                                <th scope="col" class="journal-workspace__selection-head">
                                    <input
                                        ...
                                        x-effect="
                                            ...
                                            $el.checked = visibleIds.length > 0 && count === visibleIds.length;
                            ...
                            @foreach ($exhibitions as $exhibition)
                                <tr class="{{ in_array($exhibition['id'], $selectedIds, true) ? 'is-selected' : '' }}" wire:key="exhibition-{{ $exhibition['id'] }}">
                                    <td class="journal-workspace__selection-cell">
                                        <input
                                            type="checkbox"
                                            wire:click="toggleExhibitionSelection({{ $exhibition['id'] }})"
                                            @checked(in_array($exhibition['id'], $selectedIds, true))
                                            aria-label="Toggle selection for {{ $exhibition['title'] }}"
                                        >
                                    </td>
                                    <td class="journal-workspace__identity">
                                        <strong>{{ $exhibition['title'] }}</strong>
                                        @if ($exhibition['location'])<small>{{ $exhibition['location'] }}</small>@endif
                                    </td>
                                    ...
                                    <td class="journal-workspace__schedule">
                                        @if ($exhibition['vernissage'])<em>Vernissage: {{ $exhibition['vernissage'] }}</em>@endif
                                        @if ($exhibition['date_text'] !== '')<span>{{ $exhibition['date_text'] }}</span>@endif
                                    </td>
                                    <td class="journal-workspace__actions">
                                        <div class="admin-toolbar">
                                            ...
                                            @if ($exhibition['public_url'])
                                                <a class="admin-action" href="{{ $exhibition['public_url'] }}" target="_blank" rel="noopener">View</a>
                                            @endif
                                            @switch($exhibition['state'])
                                                @case('draft')
                                                    <button class="admin-action" type="button" wire:click="publishExhibition({{ $exhibition['id'] }})">Publish</button>
                                                    @break
                                                @case('published')
                                                    <button class="admin-action" type="button" wire:click="archiveExhibition({{ $exhibition['id'] }})">Archive</button>
                                                    @break
                                                @case('archived')
                                                    <button class="admin-action" type="button" wire:click="restoreExhibition({{ $exhibition['id'] }})">Restore</button>
                                                    @break
                                            @endswitch
                                            <button class="admin-action journal-workspace__order-action" type="button" wire:click="moveExhibition({{ $exhibition['id'] }}, 'up')" @disabled(! $exhibition['can_move_up'])>↑</button>
                                            <button class="admin-action journal-workspace__order-action" type="button" wire:click="moveExhibition({{ $exhibition['id'] }}, 'down')" @disabled(! $exhibition['can_move_down'])>↓</button>
                                            <button class="admin-action is-danger" type="button" wire:click="mountAction('deleteExhibition', { exhibition: {{ $exhibition['id'] }} })" @disabled(! $exhibition['can_delete']) title="{{ $exhibition['delete_help'] ?? 'Delete exhibition' }}">Delete</button>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
                ...
                <footer class="journal-workspace__pager">
                    ...
                    <span class="journal-workspace__pager-range">
                        @if ($total === 0)0 of 0 @else{{ $resultStart }}–{{ $resultEnd }} of {{ $total }}@endif
                    </span>
