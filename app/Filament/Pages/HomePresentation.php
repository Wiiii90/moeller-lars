...
final class HomePresentation extends Page
{
    ...
    public function toggleVisibleComponentSelection(): void
    {
        $visibleTargets = collect($this->components)->pluck('target')->filter()->values()->all();
        if ($visibleTargets === []) {
            return;
        }
        $selected = collect($this->selectedComponentTargets)->filter()->unique()->values();
        if ($selected->intersect($visibleTargets)->count() === count($visibleTargets)) {
    ...
    public function sortHeroArtwork(string $target, int $position): void
    {
        ...
        $ids = collect($this->heroRailRows)->pluck('id')->map(fn ($id): int => (int) $id)->values()->all();
        $artworkId = filter_var($target, FILTER_VALIDATE_INT);
        $from = $artworkId === false ? false : array_search((int) $artworkId, $ids, true);
        if ($from === false) {
            throw ValidationException::withMessages(['manual_group' => 'The Manual Home group changed. Reload and try again.']);
        }
        $moved = $ids[$from];
    ...
    {
        if ($this->heroGroupSource !== 'manual' || ! in_array($direction, ['up', 'down'], true)) {
            return;
        }
        $ids = collect($this->heroRailRows)->pluck('id')->map(fn ($id): int => (int) $id)->values()->all();
        $index = array_search($artworkId, $ids, true);
        if ($index === false) {
            return;
        }
    ...
    public function setHeroPercentage(int $artworkId, mixed $percentage): void
    {
        if (! is_numeric($percentage)) {
            throw ValidationException::withMessages(['weight' => 'Enter a valid percentage.']);
        }
    ...
    public function sortComponent(string $target, int $position): void
    {
        ...
        $targets = collect($this->componentDataset)->pluck('target')->filter(static fn ($value): bool => is_string($value))->values()->all();
    ...
    public function moveSelectedComponents(string $direction): void
    {
        ...
        $changed = app(HomePresentationEditorialService::class)->moveSelectedComponents(
            $this->settings(), $this->componentTemplate(), $targets, $direction,
        );
        $count = count($targets);
        $this->reloadWorkspace();
        if ($changed) {
    ...
    {
        $visibleIds = $this->sourceRows()->getCollection()->pluck('id')->map(static fn ($id): int => (int) $id)->filter()->values()->all();
        if ($visibleIds === []) {
            return;
        }
    ...
    {
        $ids = collect($this->selectedSourceIds)->map(static fn ($id): int => (int) $id)->filter()->unique()->values()->all();
        if ($ids === []) {
            return;
        }
    ...
    public function sourceRows(): LengthAwarePaginator
    {
        ...
        $query = ArtworkCategory::query()->whereHas('siteSection')->with('siteSection')
            ->withCount(['artworks as published_artworks_count' => static fn ($query) => $query->where('state', 'published')])
            ->withMax(['artworks as newest_published_year' => static fn ($query) => $query->where('state', 'published')->whereNotNull('work_year')], 'work_year');
        ...
        if ($this->sourceHomeFilter === 'enabled') {
            ...
            $query->where(function (Builder $source): void {
                $source->where('show_on_home', false)
                    ->orWhereHas('siteSection', fn (Builder $section): Builder => $section->where('state', '<>', 'published'));
            });
        }
        ...
        /** @var EloquentCollection<int,ArtworkCategory> $galleries */
        $galleries = $query->orderBy('name')->offset(($page - 1) * $perPage)->limit($perPage)->get();
        $galleryIds = $galleries->map(static fn (ArtworkCategory $gallery): int => (int) $gallery->getKey())->all();

        $candidateGroup = app(HomeHeroResolver::class)->resolve($this->settings())['group'];
        ...
        $rows = $galleries->map(function (ArtworkCategory $gallery) use ($candidates): array {
            ...
            return [
                ...
                'candidates' => $galleryCandidates->take(5)->map(fn (Artwork $artwork): array => $this->candidateRow($artwork))->values()->all(),
            ];
        })->all();

        return new LengthAwarePaginator($rows, $total, $perPage, $page, ['path' => request()->url(), 'pageName' => 'sourcePage']);
    }
    ...
    private function reloadWorkspace(): void
    {
        ...
        $this->heroGroupSource = $heroConfiguration['group_source'];
        $this->heroDisplayStrategy = $heroConfiguration['display_strategy'];
        ...
        $this->manualHeroGroup = $heroConfiguration['manual_group'];
        $this->rotationIntervalCount = $heroConfiguration['rotation_interval']['count'];
        ...
        $this->componentStats = ['components' => 0, 'images' => 0, 'headings' => 0, 'rich_text' => 0, 'dividers' => 0, 'media_references' => 0];
        $this->clearComponentSelection();
        ...
        match ($template) {
            HomeTemplate::Artwork => $this->loadArtworkWorkspace($settings),
            HomeTemplate::UnderConstruction, HomeTemplate::Custom => $this->loadComponentWorkspace($template, $configuration),
    ...
    private function loadArtworkWorkspace(HomePresentationSetting $settings): void
    {
        ...
        $this->sourceGalleryCount = ArtworkCategory::query()->where('show_on_home', true)
            ->whereHas('siteSection', fn (Builder $section): Builder => $section->where('state', 'published'))->count();

        $resolution = app(HomeHeroResolver::class)->resolve($settings);
        $this->candidatePoolCount = $resolution['group']->count();
        $this->nextRotationAt = $resolution['next_rotation_at']?->toIso8601String();
        $current = $resolution['current'];
        if ($current instanceof Artwork) {
            $this->currentArtwork = $this->artworkRow($current);
        } else {
            $this->selectionIssue = $this->heroGroupSource === 'manual'
                ? 'No stored Manual group member is currently eligible.'
                : 'No eligible artwork matches the Automatic candidate settings.';
        }
        ...
        $sequence = $this->heroDisplayStrategy === 'sequential' ? $resolution['sequence'] : $resolution['group'];
        $currentId = $current instanceof Artwork ? (int) $current->getKey() : null;
        $this->heroRailRows = $sequence->values()->map(function (Artwork $artwork, int $index) use ($currentId): array {
            $row = $this->candidateRow($artwork);
            $row['eligible'] = true;
            $row['weight'] = null;
            $row['percentage'] = null;
    ...
    {
        $storedIds = array_column($this->manualHeroGroup, 'artwork_id');
        $artworks = Artwork::query()->with(['category.siteSection', 'artworkMedia.mediaAsset.variants'])
            ->whereIn('id', $storedIds)->get()->keyBy(fn (Artwork $artwork): int => (int) $artwork->getKey());
        $effectiveIds = $resolution['group']->modelKeys();
        $sequenceIds = $resolution['sequence']->modelKeys();
        $weightById = collect($this->manualHeroGroup)->mapWithKeys(fn (array $member): array => [$member['artwork_id'] => $member['weight']])->all();

        $displayIds = $this->heroDisplayStrategy === 'sequential'
            ? array_values(array_unique(array_merge($sequenceIds, $storedIds)))
            : $storedIds;
        $rows = [];
        foreach ($displayIds as $displayIndex => $id) {
            $artwork = $artworks->get($id);
            $row = $artwork instanceof Artwork ? $this->candidateRow($artwork) : [
                'id' => (int) $id,
                'title' => 'Unavailable artwork #'.$id,
                'year' => null,
                'date' => null,
                'gallery' => null,
                'thumbnail_url' => null,
            ...
            $eligible = in_array((int) $id, $effectiveIds, true);
            $sequenceIndex = array_search((int) $id, $sequenceIds, true);
            $weight = (int) ($weightById[$id] ?? 0);
            $row['eligible'] = $eligible;
            $row['weight'] = $weight;
            $row['percentage'] = number_format($weight / 100, 2, '.', '');
            $row['sequence_label'] = ! $eligible
                ? 'Unavailable'
                : ($this->heroDisplayStrategy === 'sequential'
                    ? ($sequenceIndex === 0 ? 'Current' : ($sequenceIndex === 1 ? 'Next' : 'Then'))
                    : null);
            $row['can_move_up'] = $displayIndex > 0;
    ...
    private function loadComponentWorkspace(HomeTemplate $template, array $configuration): void
    {
        ...
        $mediaIds = collect($raw)
            ->filter(fn ($component): bool => is_array($component) && ($component['type'] ?? null) === 'image' && is_numeric($component['media_asset_id'] ?? null))
            ->map(fn (array $component): int => (int) $component['media_asset_id'])->filter()->unique()->values()->all();
        $assets = MediaAsset::query()->whereIn('id', $mediaIds)->get(['id', 'original_filename', 'alt_text'])
        ...
        foreach ($raw as $index => $component) {
            ...
            $dataset[] = [
                ...
                'content' => ['primary' => $primary, 'secondary' => $secondary],
    ...
    private function projectComponents(): void
    {
        ...
        $this->components = collect($this->componentDataset)->filter(function (array $component) use ($term, $type): bool {
    ...
    {
        return [...$this->candidateRow($artwork), 'featured' => (bool) $artwork->getAttribute('featured_on_home'), 'preview_url' => route('preview.artworks.show', ['slug' => $artwork->getAttribute('slug')])];
    }
