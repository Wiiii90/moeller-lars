...
final class HomeSettingsDialog
{
    ...
    public function schema(): array
    {
        return [
            ...
            Placeholder::make('skip_target')
                ...
                ->content(fn (): string => $this->skipTargetLabel())
    ...
    private function skipTargetLabel(): string
    {
        $target = $this->resolver->skipTarget();
        if (! $target instanceof SiteSection) {
            return 'No published top-level page exists after Home. The public root safely remains on Home.';
        }

        $label = trim((string) ($target->getAttribute('navigation_label') ?: $target->getAttribute('title')));
        $path = $this->routes->path($target);
