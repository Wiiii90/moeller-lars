...
final class MediaCapacityService
{
    ...
    /**
     * Return the current presentation snapshot only when another surface has already measured it.
     * This deliberately never falls back to snapshot(), so lightweight callers cannot trigger a filesystem walk.
     *
     * @return array{configured:bool,measurement_available:bool,status:'unconfigured'|'healthy'|'near_capacity'|'full'|'unavailable',quota_bytes:int|null,authoritative_bytes:int|null,generated_bytes:int|null,managed_bytes:int|null,remaining_bytes:int|null,authoritative_ratio:float|null,original_files:int|null,generated_files:int|null,authoritative_file_bytes:array<string,int>|null}|null
     */
