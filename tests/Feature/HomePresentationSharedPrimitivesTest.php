...
it('uses reconciled shared primitives for Home data and editorial task surfaces', function (): void {
    ...
    expect($view)
        ...
        ->and($css)->toContain('.home-hero-rail__rows {')
        ...
        ->and($sharedCss)->toContain('margin: var(--admin-section-space) 0;')
        ->and($sharedCss)->toContain('.admin-visual-stage + .admin-data-controls {')
        ->and($sharedCss)->toContain('padding: var(--admin-section-space) 0 var(--admin-section-gap);')
        ->and($sharedCss)->toContain('.admin-visual-stage--stackable > .admin-visual-stage__pane {')
